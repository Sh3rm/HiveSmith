...
5. **Rule Enforcement Validation:** Verify that the new agents strictly adhere to the global rules (e.g., no destructive actions without approval, proper error handling). Confirm enforcement is structural where possible (tool allowlists, guard hooks per Rule 02 §4), not merely rhetorical (prompt pleas). In particular: a generated prompt that tells a worker to "message", "notify", or "ask" a sibling agent while that worker's `tools:` lacks `SendMessage` is a ghost-coordination finding (Rule 03 §5); a `.claude/workflows/*.js` that calls `agent()` with a type not defined in `.claude/agents/` is a ghost-infrastructure finding.
