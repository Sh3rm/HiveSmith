...
2. **Strict Schema Check (CRITICAL — Claude Code native schema):**
   ...
   - **`.claude/agents/*.md`:** Each file MUST begin with valid YAML frontmatter. REQUIRED keys: `name` and `description`. VALID OPTIONAL keys (per the official Claude Code schema): `tools`, `disallowedTools`, `model`, `permissionMode`, `maxTurns`, `skills`, `mcpServers`, `hooks`, `memory`, `effort`, `isolation`, `background`, `initialPrompt`, `color`. Do NOT flag these documented optional keys as errors — only a key outside this set is a CRITICAL `unknown_key` error. Value checks: `model` MUST be a Claude Code alias (`fable`, `opus`, `sonnet`, `haiku`), `inherit`, or a full model ID; `effort` MUST be one of `low`/`medium`/`high`/`xhigh`/`max`; `isolation` (if present) MUST be `worktree`; `background` (if present) MUST be a boolean; `permissionMode` MUST be a documented mode (`default`, `acceptEdits`, `auto`, `dontAsk`, `bypassPermissions`, `plan`, or the `manual` alias of `default`) — and `bypassPermissions` is a CRITICAL security finding unless the blueprint records explicit human approval. `tools` MUST be a comma-separated list of real Claude Code tools (e.g., `Read`, `Write`, `Edit`, `Bash`, `Glob`, `Grep`, `WebSearch`, `WebFetch`) or valid MCP tool names. A `name` value that does not match the filename is a WARNING (house convention), not a failure — Claude Code identifies agents by `name`.
