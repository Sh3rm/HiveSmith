...
2. **Strict Schema Check (CRITICAL — Claude Code native schema):** Validate every generated file against the CANONICAL schema in Rule 03 §2 (auto-loaded into your context) — required keys, the documented optional-key set, legal values for `model`/`effort`/`isolation`/`background`/`permissionMode`, and the forbidden foreign-field list. Do NOT restate or reinvent the schema; Rule 03 is the single source of truth. Check-specific severities on top of it:
   ...
   - **`.claude/settings.json`:** MUST exist, contain a `model` value equal to the blueprint's `default_model`, and — when the blueprint's `hooks` section or Rule 02 §4 applies — carry the guard-hook configuration with valid JSON structure and existing script paths. A guard hook present only in agent frontmatter (not in `settings.json`) is CRITICAL (dormant in headless runs, Rule 03 §4). `defaultMode` set to `bypassPermissions` or `auto` in a project settings file is a WARNING `ignored_setting` (Claude Code ignores it there).
