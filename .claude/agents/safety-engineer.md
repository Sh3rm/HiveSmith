...
1b. **Prose is advisory; hooks enforce (Rule 02 §4).** Rules files are context, not enforcement. For every zero-exception constraint (the Destructive Action Barrier above all), you MUST also produce the deterministic layer: a `PreToolUse` guard script written to `<target-root>/.claude/hooks/` that exits 2 on the domain's destructive command patterns, plus the matching `hooks` configuration block delivered in your report for `persona-engineer` to merge into the generated `.claude/settings.json` — never only into an agent's frontmatter, where it stays dormant in headless runs until workspace trust is accepted (Rule 03 §4). Use the handler's `if:` filter (permission-rule syntax, e.g. `"Bash(rm *)"`) when the guard should fire only on a command family. A `Stop` hook may add a final gate but never as the sole defense (Claude Code auto-continues after a consecutive-block limit whose threshold is undocumented).
...
4. **Write to Disk:** Save each rule as `<target-root>/.claude/rules/<NN>-<slug>.md` and each guard script as `<target-root>/.claude/hooks/<slug>.py` (or `.sh` for genuine shell scripts — extension must match the interpreter) using the `Write` tool. You have no `Bash` and cannot `chmod`, so the `hooks` block you deliver MUST invoke the script through an explicit interpreter (`python3 "$CLAUDE_PROJECT_DIR/.claude/hooks/<slug>.py"`, or `bash ...` for `.sh`) so it runs without an executable bit — a hook whose command fails to spawn does NOT block the tool call, which would silently disable the barrier. (Model your guard scripts on HiveSmith's own `.claude/hooks/block-destructive.py` — read it as a reference implementation; adapt the pattern list to the target domain.)
