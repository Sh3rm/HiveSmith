...
- **Key events:** `PreToolUse` (gate/deny a tool call before it runs — exit code 2 or `"permissionDecision": "deny"` blocks it), `PostToolUse`, `Stop` (gate turn-end on a passing check — note: Claude Code enforces a consecutive-block limit and auto-continues past it (threshold undocumented; historically observed around 8), so a Stop hook is a backstop, not the sole defense), `PreCompact`/`PostCompact`, `InstructionsLoaded`, `DirectoryAdded` (working directory added mid-session, ≥ 2.1.220), `WorktreeCreate`/`WorktreeRemove`. The official event list keeps growing (25+ events as of 2.1.233) — these are the swarm-relevant core; consult the official hooks reference before assuming an event does not exist.
...
Hard limits (≥ 2.1.217): default **20 concurrently-running subagents** (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`) and **200 subagent spawns per session** (`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`) — size rosters and fan-outs within these. Subagents can themselves spawn nested subagents to a default **depth of 3** (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`); strict hub-and-spoke designs must state the no-nested-delegation rule in prose (or pin the env var to 1) rather than assume the runtime forbids it. One deliberate exception to context isolation exists: **`fork` subagents** (≥ 2.1.221) inherit the parent's full conversation and prompt cache — useful for checkpoint-style continuations of the orchestrator itself, but NEVER a substitute for the explicit Manifesto when delegating to named worker agents.
...
Further official aliases exist (≥ 2.1.219): `best` (Fable 5 where available, else newest Opus), `opusplan` (Opus while planning, Sonnet for execution), `sonnet[1m]`/`opus[1m]` (1M-token context variants), and the special value `default` (clears an override back to the account default). House doctrine: generated swarms route ONLY through the five canonical tiers above — the extras are session-level conveniences for human operators, never blueprint or frontmatter values (Rule 08's `model` enum stays `fable|opus|sonnet|haiku|inherit`).
